...
class AstCreator(FileVisitor):
    ...
    @staticmethod
    def prev_token(index: int, in_list, token: str = "}"):
        # index = len(in_list)
    ...
    @staticmethod
    def last_instruction(index: int, in_list, token: str = "}"):
        for i in reversed(range(index)):
            if isinstance(
                in_list[i],
                PrintfAST
                ...
                | SwitchAST
                | CommentAST,
            ) or (isinstance(in_list[i], Node) and in_list[i].key == token):
                return i
        return -1

    @staticmethod
    def last_declaration(index: int, in_list, token: str = "}"):
        for i in reversed(range(index)):
            if isinstance(
                in_list[i],
                AssignAST
    ...
    @staticmethod
    def last_init(index: int, in_list):
        for i in reversed(range(index)):
    ...
    @staticmethod
    def last_else(index: int, in_list):
        for i in reversed(range(index)):
    ...
    @staticmethod
    def last_func_scope(index: int, in_list):
        for i in reversed(range(index)):
    ...
    @staticmethod
    def last_case_or_default(index: int, in_list):
        for i in reversed(range(index)):
    ...
    @staticmethod
    def last_switch_scope(index: int, in_list):
        for i in reversed(range(index)):
        ...
        for child in base.children[:]:
            if isinstance(child, AST):
                if child.root.key in ["expr", "term"] and child.root.value is not None:
                    ...
                    last_token = self.prev_token(index, base.children, token="{")
                    ...
                    index = base.children.index(child)
                    last_token = self.prev_token(index, base.children)
                    ...
                    last_switch_scope = self.last_switch_scope(index, base.children)
                    ...
                    last_case_default = self.last_case_or_default(index, base.children)
                    last_token = self.prev_token(index, base.children)
                    child.children = base.children[
                    ...
                    last_func = self.last_func_scope(index=index, in_list=base.children)
                    ...
                    last_case_default = self.last_case_or_default(
                    ...
                    last_decl = self.last_init(index, base.children)
                    last_decl += 1
                    ...
                    if child.parent is None:
                        child.parent = base
                    if (
                        self.prev_token(index=index, token="}", in_list=base.children)
                        == -1
                    ):
                        last_inst = self.last_instruction(index, base.children)
                ...
                elif isinstance(child, If_CondAST | While_loopAST):
                    if child.parent is None:
                        child.parent = base
                    number = 2
                ...
                elif isinstance(child, Scope_AST | FuncScopeAST):
                    # Parent of scope is base itself, if no parent is already found
                    if child.parent is None:
                        child.parent = base
                    new_index = self.prev_token(
                        index=index, token="}", in_list=base.children
                    )
                    ...
                    index = base.children.index(child)
                    if not (
                        isinstance(child.parent, If_CondAST)
                        or isinstance(child, FuncScopeAST)
                    ):
                        for ch in child.children:
                            ch.in_loop = True
                    indexes["last_instr"] = (
                        self.last_instruction(index, base.children) + 1
                    )

                elif isinstance(child, DeclrAST):
                    last_decl = self.last_declaration(index, base.children)
                    # last_decl += 1
    ...
    def dfs(self, visited, ctx, root_name: str = "math") -> AST:
        if visited is None:
            visited = []
        s = list()
    ...
    def resolve(
        ...
        while len(not_visited) > 0:
            ...
            if temp not in visited or isinstance(temp, CondAST):
                # if a scope, skip
                if not (isinstance(temp, Scope_AST | FuncScopeAST)):
                    visited.append(temp)
                if isinstance(temp, Node):
                    continue
                if (
                    not (isinstance(temp, Scope_AST | FuncScopeAST))
                    or temp is ast_in
                    or isinstance(temp.parent, Scope_AST | FuncScopeAST)
                ):
                    if isinstance(temp, While_loopAST | If_CondAST | For_loopAST):
                        visited.append(temp)
                        continue
                    ...
                    if (
                        isinstance(temp, For_loopAST)
                        and temp.initialization is not None
                    ):
    ...
    def handle(
        ...
        for ast in list_ast:
                    ...
                    if entry.name == ast.root.key:
                        ...
                        for i in range(len(entry.parameters)):
                            ...
                            if isinstance(current_arg, VarNode):
                                ...
                                if (
                                    current_param.type != getType(current_arg.value)
                                    and (
                                        getType(current_arg.value),
                                        current_param.type,
                                    )
                                    not in conversions
                                ):
            ...
            if isinstance(ast, ArrayDeclAST):
                ...
                if exists_state:
                    ...
                    if len(new_array.values) < new_array.size:
                        ...
                        for _ in range(new_array.size - len(new_array.values)):
            ...
            if len(ast.children) > 0:
                ...
                while temp_parent is not None:
                    if isinstance(temp_parent, FuncCallAST):
                        returned = True
                    if isinstance(temp_parent, While_loopAST | ReturnInstr):
                        evaluate = False
                        break
                    temp_parent = temp_parent.parent
                for child in ast.children:
                    ...
                    if (
                        isinstance(child, AST)
                        and not isinstance(ast, Scope_AST)
            ...
            if isinstance(ast, ScanfAST):
                ...
                node = ast
            # conditional cases
            elif isinstance(ast, If_CondAST | Else_CondAST):
            ...
            # declaration handling
                ...
                if not (
                    isinstance(
                        ast.children[1], Node | VarNode | ArrayNode | FuncCallAST
                    )
                ):
                    raise RuntimeError(
    ...
    def visitFile(self, ctx: FileParser.FileContext) -> AST:  # noqa: N802
        """
        Math visit function
        :param ctx: context
        :return: AST
    ...
    def visitInstr(self, ctx: FileParser.InstrContext) -> InstrAST:  # noqa: N802
        """
        Instruction visit
        :param ctx: context
        :return: AST
    ...
    def visitExpr(self, ctx: FileParser.ExprContext) -> None | ExprAST:  # noqa: N802
        """
    ...
    def visitPrintf(self, ctx: FileParser.PrintfContext) -> PrintfAST:  # noqa: N802
        """
    ...
    def visitRvar(self, ctx: FileParser.RvarContext) -> Node:  # noqa: N802
        """
    ...
    def visitRtype(self, ctx: FileParser.RtypeContext) -> Node:  # noqa: N802
        """
    ...
    def visitAssign(self, ctx: FileParser.AssignContext) -> AssignAST:  # noqa: N802
        """
    ...
    def visitDeclr(self, ctx: FileParser.DeclrContext) -> DeclrAST:  # noqa: N802
        """
    ...
    def visitVar_decl(self, ctx: FileParser.Var_declContext) -> VarDeclrAST:  # noqa: N802
        """
    ...
    def visitLvar(self, ctx: FileParser.LvarContext) -> VarNode:  # noqa: N802
        """
    ...
    def visitDeref(self, ctx: FileParser.DerefContext) -> DerefAST:  # noqa: N802
        """
    ...
    def visitTerm(self, ctx: FileParser.TermContext) -> None | TermAST:  # noqa: N802
        ast = TermAST()
    ...
    def visitFactor(self, ctx: FileParser.FactorContext) -> None | FactorAST:  # noqa: N802
        ast = FactorAST()
        ast.column = ctx.start.column
        ast.line = ctx.start.line
    ...
    def visitPrimary(self, ctx: FileParser.PrimaryContext) -> None | PrimaryAST:  # noqa: N802
        ast = PrimaryAST()
        ast.column = ctx.start.column
        ast.line = ctx.start.line
        if len(ctx.children) == 2:
    ...
    def visitScope(self, ctx: FileParser.ScopeContext) -> Scope_AST:  # noqa: N802
        out = Scope_AST(Node("unnamed", None))
        out.column = ctx.start.column
        out.line = ctx.start.line
        return out

    def visitIf_cond(self, ctx: FileParser.If_condContext) -> If_CondAST:  # noqa: N802
        out = If_CondAST(Node("If_cond", None))
        out.column = ctx.start.column
        out.line = ctx.start.line
        return out

    def visitElse_cond(self, ctx: FileParser.Else_condContext) -> Else_CondAST:  # noqa: N802
        out = Else_CondAST(Node("Else_cond", None))
        out.column = ctx.start.column
        out.line = ctx.start.line
        return out

    def visitWhile_loop(self, ctx: FileParser.While_loopContext) -> While_loopAST:  # noqa: N802
        out = While_loopAST(Node("While_loop", None))
        out.column = ctx.start.column
        out.line = ctx.start.line
        return out

    def visitFor_loop(self, ctx: FileParser.For_loopContext) -> For_loopAST:  # noqa: N802
        out = For_loopAST(Node("For_loop", None))
        out.column = ctx.start.column
        out.line = ctx.start.line
        return out

    def visitInit(self, ctx: FileParser.InitContext) -> None | InitAST:  # noqa: N802
        if len(ctx.children) == 1:
            return None
    ...
    def visitCond(self, ctx: FileParser.CondContext) -> CondAST:  # noqa: N802
        ast = CondAST()
        ast.column = ctx.start.column
        ast.line = ctx.start.line
        # cond : comp | expr
    ...
    def visitIncr(self, ctx: FileParser.IncrContext) -> TermAST | FactorAST:  # noqa: N802
    ...
    def visitCont_instr(self, ctx: FileParser.Cont_instrContext) -> ContAST:  # noqa: N802
        out = ContAST(Node("cont", None))
        out.column = ctx.start.column
        out.line = ctx.start.line
        return out

    def visitBreak_instr(self, ctx: FileParser.Break_instrContext) -> BreakAST:  # noqa: N802
        out = BreakAST(Node("break", None))
        out.column = ctx.start.column
        out.line = ctx.start.line
        return out

    def visitParam_list(self, ctx: FileParser.Param_listContext) -> FuncParametersAST:  # noqa: N802
    ...
    def visitParam_declr(self, ctx: FileParser.Param_declrContext) -> FuncParameter:  # noqa: N802
        out = FuncParameter(
            key=ctx.var.text,
            value=None,
    ...
    def visitFunc_defn(self, ctx: FileParser.Func_defnContext) -> FuncDefnAST:  # noqa: N802
        out = FuncDefnAST(
    ...
    def visitFunc_decl(self, ctx: FileParser.Func_declContext) -> FuncDeclAST:  # noqa: N802
        out = FuncDeclAST(
            root=Node(ctx.name.text, None),
            const=(ctx.const is not None),
            return_type=ctx.type_.text,
    ...
    def visitFunc_arg(self, ctx: FileParser.Func_argContext) -> None:  # noqa: N802
        return

    def visitArg_list(self, ctx: FileParser.Arg_listContext) -> None:  # noqa: N802
        """
        :return: Node with name args_list and value the number of arguments
        """
        return

    def visitFunc_call(self, ctx: FileParser.Func_callContext) -> FuncCallAST:  # noqa: N802
        """
    ...
    def visitFunc_scope(self, ctx: FileParser.Func_scopeContext) -> FuncScopeAST:  # noqa: N802
        """
        ...
        """
        out = FuncScopeAST(
            Node(ctx.parentCtx.name.text, None), symbolTable=SymbolTable()
        )
        out.symbolTable.owner = out
        out.column = ctx.start.column
        out.line = ctx.start.line
    ...
    def visitReturn_instr(self, ctx: FileParser.Return_instrContext) -> ReturnInstr:  # noqa: N802
    ...
    def visitScanf(self, ctx: FileParser.ScanfContext) -> ScanfAST:  # noqa: N802
    ...
    def visitArray_decl(self, ctx: FileParser.Array_declContext) -> ArrayDeclAST:  # noqa: N802
        ast = ArrayDeclAST(
            VarNode(
                ctx.name.text + "[]",
                None,
    ...
    def visitArray_el(self, ctx: FileParser.Array_elContext) -> ArrayElementAST:  # noqa: N802
    ...
    def visitIncl_stat(self, ctx: FileParser.Incl_statContext) -> IncludeAST:  # noqa: N802
    ...
    def visitComp(self, ctx: FileParser.CompContext) -> ExprAST | TermAST:  # noqa: N802
    ...
    def visitSwitch_instr(self, ctx: FileParser.Switch_instrContext) -> SwitchAST:  # noqa: N802
    ...
    def visitCase_instr(self, ctx: FileParser.Case_instrContext) -> CaseAST:  # noqa: N802
        out = CaseAST(Node("case", None))
        out.column = ctx.start.column
        out.line = ctx.start.line
        return out

    def visitDefault_instr(self, ctx: FileParser.Default_instrContext) -> DefaultAST:  # noqa: N802
        out = DefaultAST(Node("default", None))
        out.column = ctx.start.column
        out.line = ctx.start.line
        return out

    def visitSwitch_scope(self, ctx: FileParser.Switch_scopeContext) -> SwitchScopeAST:  # noqa: N802
        out = SwitchScopeAST(Node("switch_scope", None))
        out.column = ctx.start.column
        out.line = ctx.start.line
        return out

    def visitComment(self, ctx: FileParser.CommentContext) -> CommentAST:  # noqa: N802
    ...
    @staticmethod
    def convert(
        value: int | str | float | None, d_type: CastType
    ) -> None | int | float | str:
        """
        ...
        try:
            if value is None:
                return None
            if d_type == CastType.INT:
                if isinstance(value, int):
                    return value
                if isinstance(value, str):
                    return ord(value)
                return int(value)
            if d_type == CastType.FLOAT:
                if isinstance(value, float):
                    return value
                if isinstance(value, str):
                    return float(ord(value))
                return float(value)
            if d_type == CastType.CHAR:
                if isinstance(value, str):
                    return value
                if value == 0:
                    return value
